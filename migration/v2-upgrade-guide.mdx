...
description: 'Complete guide for migrating from Bubble Tea v1 to v2'
---
...
<Steps>
  ...
  <Step title="Update View method signature">
    Change `View() string` to `View() tea.View`
  </Step>
  <Step title="Update key message handling">
    Replace `tea.KeyMsg` with `tea.KeyPressMsg`
  </Step>
  <Step title="Update key fields">
    Change `msg.Type` to `msg.Code`, `msg.Runes` to `msg.Text`, etc.
  </Step>
  <Step title="Fix space key handling">
    Replace `case " ":` with `case "space":`
  </Step>
  <Step title="Update mouse messages">
    Update mouse message types and method calls
  </Step>
  <Step title="Rename mouse button constants">
    Update button constant names
  </Step>
  <Step title="Remove old program options">
    Replace with View fields
  </Step>
  <Step title="Remove imperative commands">
    Replace with View fields
  </Step>
  <Step title="Remove old program methods">
    Update to use View fields or new methods
  </Step>
  <Step title="Rename APIs">
...
In v1, you'd use program options like `tea.WithAltScreen()` and commands like `tea.EnterAltScreen` to toggle terminal features on and off. In v2, you just set fields on the `tea.View` struct in your `View()` method and Bubble Tea handles the rest.
...
```go v1: imperative — scattered across NewProgram, Init, and Update
p := tea.NewProgram(model{}, tea.WithAltScreen(), tea.WithMouseCellMotion())
```

```go v2: declarative — everything lives in View()
...
```go Handling both presses and releases
case tea.KeyMsg:
    switch key := msg.(type) {
    case tea.KeyPressMsg:
        // key press
...
```go After (option A — string matching)
case tea.KeyPressMsg:
    switch msg.String() {
    case "ctrl+c":
        // ctrl+c
...
```go After (option B — field matching)
case tea.KeyPressMsg:
